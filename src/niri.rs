...
use niri_config::appearance::DEFAULT_BACKGROUND_COLOR;
...
impl Niri {
    ...
    pub fn render_inner<R: NiriRenderer>(
        ...
        let has_fullscreen_background = zoom < 1.0
            && self
                .layers_in_render_order(&layer_map, Layer::Background, false, false)
                .any(|(mapped, geo)| mapped.is_fullscreen_background(geo))
            || zoom < 1.0
                && self
                    .layers_in_render_order(&layer_map, Layer::Background, true, false)
                    .any(|(mapped, geo)| mapped.is_fullscreen_background(geo));
    ...
    fn render_backdrop_image<R: NiriRenderer>(
        ...
        for (mapped, geo) in self.layers_in_render_order(layer_map, Layer::Background, false, false)
        {
    ...
    fn render_layer_normal<R: NiriRenderer>(
        ...
        for (mapped, geo) in self.layers_in_render_order(layer_map, layer, for_backdrop, false) {
